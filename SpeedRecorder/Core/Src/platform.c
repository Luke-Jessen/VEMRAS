...
static HAL_StatusTypeDef SPIx_TransceiveBytes(SPI_HandleTypeDef* handle, GPIO_TypeDef* chipSelectPort, uint16_t chipSelectPin, uint16_t numBytes, uint16_t timeout, uint8_t* txData, uint8_t* rxData);
    ...
    switch (interface->interfaceType)
    {
        ...
        case WE_spi:
            ...
            else
            {
                for (uint16_t i = 0; (i < numBytesToRead) && (status == HAL_OK); i++)
                {
                    status = SPIx_ReadBytes((SPI_HandleTypeDef*)interface->handle, (GPIO_TypeDef*)interface->options.spi.chipSelectPort, interface->options.spi.chipSelectPin, regAdr + i, 1, interface->options.readTimeout, data + i);
                }
            }
#else
            status = HAL_ERROR;
    ...
    switch (interface->interfaceType)
    {
        ...
        case WE_spi:
            ...
            else
            {
                for (uint16_t i = 0; (i < numBytesToWrite) && (status == HAL_OK); i++)
                {
                    status = SPIx_WriteBytes((SPI_HandleTypeDef*)interface->handle, (GPIO_TypeDef*)interface->options.spi.chipSelectPort, interface->options.spi.chipSelectPin, regAdr + i, 1, interface->options.writeTimeout, data + i);
                }
            }
#else
            status = HAL_ERROR;
 ...
 * @param[in] chipSelectPin Pin used for chip select
 * @param[in] numBytes Number of bytes to write and read
 * @param[in] timeout Timeout for the SPI operation
 * @param[in] txData Pointer to transmit data buffer
 * @param[out] rxData Pointer to receive data buffer
 * @retval HAL status
 */
static HAL_StatusTypeDef SPIx_TransceiveBytes(SPI_HandleTypeDef* handle, GPIO_TypeDef* chipSelectPort, uint16_t chipSelectPin, uint16_t numBytes, uint16_t timeout, uint8_t* txData, uint8_t* rxData)
{
    ...
    status = HAL_SPI_TransmitReceive(handle, txData, rxData, numBytes, timeout);
    if (status != HAL_OK)
    {
        return status;
    }
